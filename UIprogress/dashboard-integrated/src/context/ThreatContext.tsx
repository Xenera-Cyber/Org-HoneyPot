...
const sourceData = [
  { ip: "185.194.12.55", country: "RU", protocol: "SSH", attacks: 84, risk: 94 },
  { ip: "103.44.12.17", country: "CN", protocol: "HTTP", attacks: 71, risk: 88 },
  { ip: "91.77.13.201", country: "US", protocol: "FTP", attacks: 53, risk: 74 },
  { ip: "178.14.2.16", country: "DE", protocol: "SMTP", attacks: 42, risk: 69 },
  { ip: "43.201.87.2", country: "IN", protocol: "SSH", attacks: 36, risk: 61 },
];

const attackTypeData = [
  { name: "SSH", percentage: 46, color: "bg-cyan-500" },
  { name: "HTTP", percentage: 28, color: "bg-green-500" },
  { name: "FTP", percentage: 14, color: "bg-yellow-500" },
  { name: "SMTP", percentage: 8, color: "bg-orange-500" },
  { name: "OTHER", percentage: 4, color: "bg-purple-500" },
];

const liveThreatData: LiveThreat[] = [
  { id: 1, ip: "185.194.12.55", protocol: "SSH", severity: "HIGH", confidence: 97, status: "Active", action: "Deploy Deception" },
  { id: 2, ip: "103.44.12.17", protocol: "HTTP", severity: "MEDIUM", confidence: 91, status: "Monitoring", action: "Observe" },
  { id: 3, ip: "91.77.13.201", protocol: "FTP", severity: "LOW", confidence: 82, status: "Contained", action: "Sandbox" },
  { id: 4, ip: "178.14.2.16", protocol: "SMTP", severity: "CRITICAL", confidence: 99, status: "Blocked", action: "Isolate" },
];

const statusCycle: string[] = ["Active", "Monitoring", "Contained", "Blocked"];
const severityCycle: ("LOW" | "MEDIUM" | "HIGH" | "CRITICAL")[] = [
  "LOW",
  "MEDIUM",
  "HIGH",
  "CRITICAL",
...
const scoreTimelineDemo: ScoreTimelinePoint[] = Array.from(
  { length: 24 },
  (_, i) => ({
...
const DEMO_STATE: Omit<ThreatContextData, "isLive"> = {
  threatScore: 82,
  activeThreats: 37,
  blockedIPs: 142,
  aiConfidence: 96,
  heatmap: initialHeatmap,
  scoreTimeline: scoreTimelineDemo,
...
export function ThreatProvider({ children }: { children: ReactNode }) {
  ...
  const [stats, setStats] = useState<Omit<ThreatContextData, "isLive">>(
    DEMO_STATE
  );
  const [isLive, setIsLive] = useState(false);

  // Poll dashboard_api.py, which derives all of this from real honeypot
  ...
  useEffect(() => {
    ...
    async function poll() {
      const live = await apiGet<Omit<ThreatContextData, "isLive">>(
        "/api/threats"
      );
      if (cancelled) return;
  ...
  // Demo-mode simulation, only while no real backend is connected.
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setStats((prev) => ({
        ...
        scoreTimeline: prev.scoreTimeline.map((item) => ({
          ...item,
          score: Math.max(
            0,
            Math.min(100, item.score + Math.floor(Math.random() * 9 - 4))
          ),
        })),
        ...
        liveThreats: prev.liveThreats.map((item) => {
          ...
          if (shouldChangeState) {
            ...
            const nextSeverityIndex =
              (currentSeverityIndex + (Math.random() > 0.5 ? 1 : -1) + severityCycle.length) %
  ...
  return (
    <ThreatContext.Provider value={{ ...stats, isLive }}>
      {children}
